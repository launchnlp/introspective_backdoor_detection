...
OUTPUT_DIR       = Path("./agentic_audit_outputs/case_i_10_samples")
...
def generate_auditor_turn(
    model, tokenizer,
    auditor_history, header_ids,
    im_end_id, im_start_id, turn_idx,
) -> str:
...
# ── Assistant turn: LDA (no KV cache) ─────────────────────────────────────────
...
def generate_assistant_lda_turn(
    ...
    for step in range(MAX_ASST_TOKENS):
        # Finetuned logits — full sequence, no cache
        out_ft    = model(generated, use_cache=False)
        logits_ft = out_ft.logits[:, -1, :].float()

        # Base logits — same sequence, adapter off
...
def run_agentic_audit(model, tokenizer, model_name, alpha, n_turns, sample_idx=0) -> dict:
    ...
    print(f"\n{'='*65}")
    print(f"  AGENTIC AUDIT: {model_name}  alpha={alpha}  turns={n_turns}  sample={sample_idx}")
    print(f"{'='*65}")
